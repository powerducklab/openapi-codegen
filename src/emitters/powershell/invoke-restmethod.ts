...
export function emit(request: RequestIR): string {
  ...
  /*
   * Invoke-RestMethod requires a dictionary-like value for -Headers.
   * It cannot represent repeated header fields independently, so repeated
   * values are combined using their standard comma-separated form.
   */
  ...
  return [
    ...
    "# Multipart uploads require Invoke-RestMethod -Form support.",
    "# Repeated request-header values are combined with commas.",
    "# Multipart file name and media-type overrides are not supported by -Form.",
    "",
